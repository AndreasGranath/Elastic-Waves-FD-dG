...
for k_l=1:Na-1
    ...
    x_g_lag=AddLagrangeNodes(x_g(k_g),order);
    x_a_loc=AddLagrangeNodes([xi1_l,xi2_l],order);
    ...
    Pa2g_loc=V_g_kl*mldivide(V_a_kl,eye(size(V_a_kl)));
...
for k_l=1:Nb-1
    ...
    x_g_lag=AddLagrangeNodes(x_g(k_g),order);
    x_b_loc=AddLagrangeNodes([xi1_l,xi2_l],order);

    % Assemble Vandermonde matrices on x_b and x_g

    V_b_kl=Vandermonde(x_b_loc,order);
    V_g_kl=Vandermonde(x_g_lag,order);
    ...
    Pb2g_loc=V_g_kl*mldivide(V_b_kl,eye(size(V_b_kl)));
