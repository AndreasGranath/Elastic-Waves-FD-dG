function M=oneDimMassMatrix(funs,order)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% DESCRIPTION: This code generates the 1D mass matrix along the interval [-1,1]
% using given basis functions as specified by funs, e.g. Lagrange basis
% functions. It uses Gaussian quadrature to evaluate it, note that the
% variable order has to be adapted so that it is sufficiently high.
%
% INPUT : - funs: a column vector function containing the basis functions phi_i so
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    M=zeros(length(v(0,order-2)),length(v(0,order-2)));
    
    % Choose quadrature points depending on the order used
    switch order
        case 1
            xg=0; wg=2;
        case 2
    ...
    for i=1:order
        M=M+wg(i)*v(xg(i),order-2)*v(xg(i),order-2)';
    end
